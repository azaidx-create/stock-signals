...
the results into docs/index.html.

That file gets published through GitHub Pages, so you can open one
bookmarked link anytime to see the latest check.
"""
...
def get_market_uptrend():
    ...
    if spy.empty or len(spy) < 200:
        raise RuntimeError(
            "Not enough S&P 500 data to determine the market trend."
        )
...
def check_ticker(ticker):
    ...
    distance_above_sma50_pct = (
        (price - sma50_today) / sma50_today
    ) * 100

    near_sma50 = (
        0
        <= distance_above_sma50_pct
        <= MAX_DISTANCE_ABOVE_SMA50_PCT
    )
    ...
    return {
        ...
        "distance_above_sma50_pct": distance_above_sma50_pct,
...
def build_telegram_message(buy_rows, checked_at):
    ...
    for row in buy_rows:
        ...
        lines.extend(
            [
                ...
                (
                    "Distance above 50-day average: "
                    f"{row['distance_above_sma50_pct']:.2f}%"
                ),
                (
                    "Reason: Long-term uptrend with price "
    ...
    lines.append(
        "Review the price in TradingView "
...
def build_html(
    ...
    for row in rows:
        ...
        if status == "buy":
            badge = (
                '<span style="background:#35e08a;'
                'color:#05130b;padding:6px 12px;'
                'border-radius:6px;font-weight:700;">'
                "BUY SIGNAL</span>"
            )

        elif status == "wait_uptrend":
            badge = (
                '<span style="color:#8a948d;">'
                "WAIT — uptrend but not near "
                "the 50-day average</span>"
            )

        elif status == "avoid":
            badge = (
                '<span style="color:#e05555;">'
                "AVOID — below long-term trend</span>"
            )

        elif status == "skipped_fundamentals":
            badge = (
                '<span style="color:#8a948d;">'
                "skipped — weak fundamentals</span>"
            )
        ...
        if status == "buy":
            ...
            target_txt = (
                f'<b style="color:#4ba3ff;">'
                f'${row["price"] * 1.20:.2f}</b>'
            )
        ...
        elif row.get("distance_above_sma50_pct") is not None:
            entry_txt = "—"
            stop_txt = "—"
            target_txt = "—"
...
<body>
  ...
  <div class="stats">
    ...
    <div class="card">
      <div class="label">Passed fundamentals</div>
      <div class="value">
        {stats['fundamentals_passed']}
      </div>
    </div>

    <div class="card">
      <div class="label">Rejected fundamentals</div>
      <div class="value">
        {stats['fundamentals_rejected']}
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="value">
        {stats['fundamentals_unavailable']}
      </div>
    </div>

    <div class="card">
      <div class="label">Price data scanned</div>
      <div class="value">{stats['price_scanned']}</div>
    </div>

    <div class="card">
      <div class="label">No usable data</div>
      <div class="value">{stats['no_data']}</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="value">{stats['near_sma50']}</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="value">{stats['wait_uptrend']}</div>
    </div>

    <div class="card">
      <div class="label">Buy signals</div>
      <div class="value">{stats['buy']}</div>
    </div>
  </div>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Price</th>
          <th>Status</th>
          <th>Distance from SMA50</th>
          <th>Entry price</th>
          <th>Stop-loss (-10%)</th>
          <th>Take-profit (+20%)</th>
        </tr>
      </thead>
  ...
  <footer>
    ...
    to be between 0% and
    {MAX_DISTANCE_ABOVE_SMA50_PCT:.0f}% above its 50-day
    moving average.

    Scheduled scans run at 9:45 PM UTC, which is 12:45 AM
    Saudi time on the following calendar day. During US
    daylight-saving time this is 5:45 PM New York; during
    standard time it is 4:45 PM New York.
        ...
        rows.append(
            {
                ...
                "status": status,
                "distance_above_sma50_pct":
                    result["distance_above_sma50_pct"],
            }
        )
