...
def build_telegram_message(buy_rows, checked_at):
    ...
    lines.append(
        "Review the price in TradingView before placing a paper trade."
    )
...
def build_html(market_up, rows, checked_at):
    ...
    for row in rows:
        if row["status"] == "buy":
            badge = (
                '<span style="background:#35e08a;color:#05130b;'
                'padding:6px 12px;border-radius:6px;font-weight:700;">'
                'BUY SIGNAL</span>'
            )

        elif row["status"] == "wait_uptrend":
            badge = (
                '<span style="color:#8a948d;">'
                'WAIT — already in uptrend</span>'
            )

        elif row["status"] == "avoid":
            badge = (
                '<span style="color:#e05555;">'
                'AVOID — below trend</span>'
            )

        elif row["status"] == "skipped_fundamentals":
            badge = (
                '<span style="color:#8a948d;">'
        ...
        row_html += f"""
        <tr>
          <td style="padding:12px 10px;font-weight:700;border-bottom:1px solid #232a26;">
            {row['ticker']}
          </td>

          <td style="padding:12px 10px;border-bottom:1px solid #232a26;">
            {price_txt}
          </td>
          ...
          <td style="padding:12px 10px;border-bottom:1px solid #232a26;color:#e0a835;">
            {stop_txt}
          </td>
...
<body>
  ...
  <div class="sub">
    Last checked automatically: {checked_at}.
    Updates once per weekday — no need to press anything.
  </div>
  ...
  <footer>
    This reflects a strategy tested to show roughly a 55-60% win rate
    in normal markets, and losses during prolonged downturns like
    2000-2002.

    The take-profit target (+20%) is a standard 2:1 reward-to-risk
    rule of thumb, not a number we specifically backtested as optimal
    — a sensible default, not a proven target.

    This page updates automatically once a day — no action needed
    from you.
  </footer>
...
def main():
    ...
    checked_at = (
        checked_at_saudi.strftime("%Y-%m-%d %I:%M %p")
        + " Saudi time"
    )
